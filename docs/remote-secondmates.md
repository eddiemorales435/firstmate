...
The Herdr agent starts that server through the remote account's login shell (`-l -c`, from Directory Services `UserShell` on macOS) so the Aqua login session receives login-shell environment and login-keychain access, and `exec` keeps the server in the foreground under launchd rather than letting it detach into a Background session.
